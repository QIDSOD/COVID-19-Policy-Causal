...
parser.add_argument('--type_y', type=str, default='confirmed')
parser.add_argument('--type_net', type=str, default='dist')
parser.add_argument('--history', type=bool, default=True)

parser.add_argument('--alpha', type=float, default= -1, help='reverse layer, multiply by a constant')
...
def train(epochs, x, adj_dense_list, treat_his, treat_cur, y, y_hist, idx_trn, idx_tst, model, optimizer):
    ...
    for k in range(epochs):  # epoch
        ...
        # forward
        y1_pred_list, y0_pred_list, all_z_t, all_ps, h = model(x, adj_dense_list,
                                                               treat_cur, y_hist)
...
def test(x, adj_dense_list, treat_his, treat_cur, y, y_hist, idx_tst, model):
    ...
    y1_pred_list, y0_pred_list, _, _, _ = model(x, adj_dense_list, treat_cur, y_hist)  # T x test_size x 1
...
def generate_time(time_start, time_end, interval):
    time_step_list = []
    ...
    while date_s + datetime.timedelta(days=interval) < end:
        date_t = date_s + datetime.timedelta(days=interval)
        time_step_list.append([date_s, date_t])
        date_s = date_t + datetime.timedelta(days=1)
...
if __name__ == '__main__':
    ...
    time_step_list = generate_time("2020-01-22", "2020-12-31", 15)
    ...
    policy_keywords = {"SD": ["social distance", "social distancing", "gather", "remote", "close"],
                       ...
                       "MA": ["mask", "face covering", "Mask", "face", "Face"]}
    ...
    results_dict = {}
    ite_results_dict = {}
    for policy_cate_interest in cate_list:
        treat = treat_orin.copy()

        results_dict[policy_cate_interest] = {}  # {'SD': {}}
        ite_results_dict[policy_cate_interest] = {}
        for policy_spe_interest in policy_micro[policy_cate_interest]:
            if policy_spe_interest is None:
                policy_select = policy_index.index(policy_cate_interest)
                treat = treat[:, :, policy_select]  # category-level, N x T_old
            else:
                ...
                treat = treat.swapaxes(0, 1)  # N x T_old
            ...
            model = CovidRNN(x_dim, y_dim, t_dim, y_hist_dim, args)
