...
def top_policy(path, time_step_list=None, level='state', policy_keywords=None, policy_index=None, countys=None, mustcontain=None):
    ...
    policy_details = {cate: {} for cate in policy_index}  # {'SO': {'food and drink': {state: start/stop/None, assign: T x N assignment}}}
    ...
    # for each category, select top k policies
    topk = 100
    policy_assign_all = {}
    policy_names_all = {}
    for cate in policy_index:
        ...
        policy_assign_cate = np.concatenate(policy_assign_cate, axis=0)  # |Cluster size| x T x N
        policy_num_cate = np.sum(policy_assign_cate, axis=-1)  # |Cluster size| x T, num of counties applied
        sort_idx_cate = np.argsort(policy_num_cate[:, -1])[::-1]  # |Cluster size|, sort by the assigned number in the last time step in descending order
